...
DATASET="$TRAIN_DIR/beat-instrument-finetune.jsonl"
CONFIG="$TRAIN_DIR/beat-instrument-qwen-lora.yaml"

if [[ ! -f "$DATASET" ]]; then
  echo "Missing dataset: $DATASET"
  echo "Export it from Beat Preferences -> Training Data -> Export instruments."
  exit 1
fi
