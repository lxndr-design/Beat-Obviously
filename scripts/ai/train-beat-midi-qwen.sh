...
DATASET="$TRAIN_DIR/beat-midi-song-finetune.jsonl"
CONFIG="$TRAIN_DIR/beat-midi-qwen-lora.yaml"

if [[ ! -f "$DATASET" ]]; then
  echo "Missing dataset: $DATASET"
  echo "Export it from Beat Preferences -> Training Data -> Export MIDI."
  exit 1
fi
