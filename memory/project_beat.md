...
- Beat has a working React/Vite UI preview at `http://127.0.0.1:4173/` and a JUCE standalone build in `build-native`.
...
- The most recent native-engine foundation fix wires `engine.applyProject`: React now sends the current project model to JUCE, and `backend/Source/Ipc/MessageBridge.cpp` parses frontend project JSON into native `Project`, `Track`, `Segment`, MIDI notes, transpose, time signature, and EQ automation.
...
- Native sampler foundation: `engine.applyProject` now sends instrument definitions from React to JUCE; backend stores instrument sample URLs, resolves bundled `/samples/...` paths inside the app resources, preloads sample buffers, carries instrument IDs through trigger events, and plays sample instruments through a simple native round-robin sample voice path before falling back to the synth voice.
