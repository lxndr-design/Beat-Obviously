...
- A deterministic non-AI resynthesis helper can derive an initial four-frame wavemap from audio samples by slicing the source and mapping energy, zero-crossing rate, derivative motion, and polarity asymmetry into brightness, even-harmonic amount, fold, formant, notch, skew, and phase controls. The Solid oscillator editor exposes this through an `Import Audio` action in the wavemap panel. Browser preview decodes data-URL imports locally, while packaged native builds call `instrument.resynthesizeWavemap` so local file paths stay behind the IPC boundary.
