...
- Audio segments may carry an optional `tunePitch`. MIDI C4 is the neutral
  source pitch; each semitone above or below changes the source playback ratio
  by `2^((tunePitch - 60) / 12)`. Editor preview, arrangement playback, and
  offline export use the same rate, while trim and split operations convert
  timeline beats back into tune-adjusted source offsets.
...
- Native project persistence now roundtrips preset-grade Aether instrument state instead of only the legacy `hasAether` flag. `ProjectRepository` serializes Aether oscillator A/B, sub, noise, wavetable frame controls, 16-bin custom-frame partial overlays, morph/interpolation flags, macro values, dynamic macro/modulation routes, Env 2, and instrument-owned FX. Older rows that only carry `hasAether` load with a bounded default Aether config rather than a silent partial instrument, and backend stress verifies save/load plus rendered output for the full shape.
...
- Aether custom tables now use a first-class wavemap metadata shape in synth patches. `metadata.wavemaps` is the forward key, while `metadata.customWavetables` remains a legacy alias for older saved patches. Wavemaps carry schema version, source/import metadata, interpolation intent, stable frame ids/positions, and the harmonic frame controls used by the current native custom wavetable renderer.
- A deterministic non-AI resynthesis helper can derive an initial four-frame wavemap from audio samples by slicing the source and mapping energy, zero-crossing rate, derivative motion, polarity asymmetry, and bounded 16-bin harmonic projection into brightness, even-harmonic amount, fold, formant, notch, skew, spectral tilt, resonance focus, phase, and drawn partial controls. Resynthesized wavemaps also opt into a bounded morph amount so scans between frames are animated without changing older zero-morph patches. The Solid oscillator editor exposes this through an `Import Audio` action in the wavemap panel, including full, transient, sustain, and visible manual source-range selection. Browser preview decodes data-URL imports locally, while packaged native builds call `instrument.resynthesizeWavemap` so local file paths stay behind the IPC boundary.
- Custom wavemap frames may also carry a 16-bin `partials` overlay for manual harmonic drawing. The Solid oscillator editor exposes a compact per-frame bar editor, and browser/native custom table generation interpolates those partial bins with the rest of the frame controls so preview, live playback, and export share the same drawn harmonic intent.
...
- The Solid frontend sends `app.ready` only after instruments, components, and audio files have completed their startup load attempts.
