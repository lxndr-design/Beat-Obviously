...
| Three source slots | Fixed three-slot interface exists; Slot 1 is product-connected and Slots 2-3 are reserved | C1/C2A | Preserve the interface while each later source receives its own lifecycle, budget, persistence, and transition gate |
| Sample engine | Slot 1 reuses Beat's decoded project-asset cache; schema-v3/v2 migration adds normalized start/end slicing and optional click-bounded forward looping; missing-asset handling, per-note pitch, level/pan, filter/direct routing, bounded replacement transition, native UI, and live/offline parity pass | C1/C2A/C2B | Add mapped zones, streaming strategy, sample-source FX sends, reverse/ping-pong policy only if justified, and broader audible replacement automation before calling it a full sampler |
