...
| Multisample/SFZ | DecentSampler zones plus disconnected Beat-owned C3A parsing, C3B resolution, and C3C1 bounded immutable decoding; canonical containment, post-open metadata checks, duplicate sharing, strict file/decoded-memory/candidate caps, deterministic WAV tests, and explicit diagnostics pass without upstream code or a dependency | C3A/C3B/C3C1 | C3C2: descriptor-level identity or digest policy plus lifetime-safe control-thread publication/deferred destruction; keep sequence metadata inactive until round-robin policy is separately approved |
