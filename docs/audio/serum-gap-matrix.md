...
| Sample engine | Slot 1 has normalized slicing, click-bounded forward looping, eight fixed key/velocity zones, deterministic two-zone overlap blending, per-zone editing, two FX sends, and conservative live streaming for Aether-only files >= 8 s; one shared worker, fixed double-bank caches, attack/loop preloads, 64-sample underflow/recovery fades, aggregate telemetry, decoded shared-asset fallback, and explicit full-decode offline mode pass | C1/C2A/C2B/C2C/C2D/C2E/C2F/C2G/C2H | Add round-robin only with a separate policy; measure sustained disk-starvation behavior on representative hardware before broadening eligibility or page budgets |
