...
| Sample engine | Slot 1 reuses Beat's decoded project-asset cache; schema-v5/v4 migration adds normalized slicing, click-bounded forward looping, eight fixed key/velocity zones, deterministic two-zone equal-power overlap blending, per-zone editing, and two fixed shared-FX sends; missing-asset handling, routing, replacement, callback safety, and live/offline parity pass | C1/C2A/C2B/C2C/C2D/C2E/C2F | Add streaming strategy, round-robin only with a separate explicit policy, and broader audible automation before calling it a full sampler |
