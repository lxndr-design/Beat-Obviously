...
The browser oscillator probe's former `-6.878 dB` / `-6.424 dB` values were filter-contaminated: changing the persisted flag after preview construction never bypassed the preview filter. An explicit backward-compatible runtime bypass, common full-band wavetable normalization, a sharper 513-tap reference decimator, and gain/correlation diagnostics produced `-26.180 dB` / `-26.333 dB` with approximately `0.999` correlation and unit fitted gain. The project owner accepted that Nyquist-edge behavior for Serum 1. The fixed unison capacity is now 16 throughout native/frontend live, offline, persistence, and preview paths; the supplied Future Bass benchmark renders its authored nine voices. Its refreshed isolated residual is `-26.194 dB`, with 99.8834% of residual energy in 16–24 kHz. The browser reference-render hot path was then optimized from `0.591x` to `1.033x` realtime for Future Bass with a 42.8% wall-time reduction and exact preservation of all frozen hashes; native callback performance and code are unchanged. Full native/non-native gates pass with only the existing TCC waiver. These decisions close the remaining Serum 1 acceptance rows; Serum 2 comparison work may begin as a separate scope.
