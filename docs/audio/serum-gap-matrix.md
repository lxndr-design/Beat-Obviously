...
| FM/PM/PD/AM/ring modulation | Bounded A-by-B AM and ring modes implemented; a disconnected fixed 2x/source-rate-render candidate reduces the measured AM/ring alias cases below 0.005 without callback-safety violations; FM/PM/PD remain absent | B13/B26 | Integrate source-rate oversampling into the production A/B renderer without changing the off/zero path, phase lifecycle, routing, or budgets; then rerun all release gates |
