...
| Multisample/SFZ | DecentSampler zones plus disconnected Beat-owned C3A parsing and C3B sample resolution; canonical-root containment, symlink escape/type/existence checks, 4 GiB per-file / 16 GiB aggregate / 32-candidates-per-note caps, stable immutable 128-note indexing, and explicit diagnostics pass without upstream code or a dependency | C3A/C3B | C3C: reviewed decode and lifetime-safe publication with post-open identity/containment revalidation; keep sequence metadata inactive until round-robin policy is separately approved |
