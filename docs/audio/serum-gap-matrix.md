...
| MPE/per-note expression | Independent mod wheel, poly/channel pressure, CC74 timbre, per-channel RPN 0,0 pitch ranges, a versioned persisted contiguous master/member zone, and legacy lower/upper RPN 0,6 runtime negotiation; fixed channel caches initialize Aether voices deterministically and survive cross-channel steals | B3/B14/B18/B19/B20/B21 | Add MIDI-CI/profile negotiation, simultaneous-zone policy, automatic MCM pitch-range defaults, product controls, and non-Aether initialization before claiming MPE completeness |
