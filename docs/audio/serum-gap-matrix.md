...
| Sample engine | Slot 1 reuses Beat's decoded project-asset cache; schema-v4/v3 migration adds normalized slicing, click-bounded forward looping, and eight fixed key/velocity zones with deterministic narrowest-span selection; missing-asset handling, routing, replacement, UI, and live/offline parity pass | C1/C2A/C2B/C2C | Add streaming strategy, sample-source FX sends, cross-zone blending/round-robin only with explicit policy, per-zone advanced product editing, and broader audible automation before calling it a full sampler |
