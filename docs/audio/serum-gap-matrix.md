...
The remaining Serum 1 benchmark gaps are now narrow and explicit: the reference-subtracted oscillator cross-rate residual remains `-6.878 dB` / `-6.424 dB`, and the engine retains a fixed eight-voice unison capacity while the supplied Future Bass description requested nine. Serum 2 comparison work remains deferred until those gaps are corrected or explicitly accepted.
