...
| Sample engine | Slot 1 reuses Beat's decoded project-asset cache, has schema-v2 patch migration, missing-asset handling, per-note pitch, level/pan, filter/direct routing, bounded replacement transition, native UI, and live/offline parity | C1/C2A | Add loop/slicing policy, mapped zones, streaming strategy, sample-source FX sends, and broader audible replacement automation before calling it a full sampler |
