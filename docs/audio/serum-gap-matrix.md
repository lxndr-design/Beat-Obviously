...
| Multisample/SFZ | DecentSampler zones plus a disconnected Beat-owned C3A SFZ subset parser/model; strict inheritance, path/range/loop/sequence validation, explicit unsupported diagnostics, and 1 MiB / 4,096-opcode / 256-region / 512-detail limits pass without upstream code or a dependency | C3A | C3B: safe sample-root resolution and immutable bounded per-note region index; keep sequence metadata inactive until round-robin policy is separately approved |
