...
| Callback-safety proof | Dense warmed engine callback has zero allocations under a test-only global C++ allocation interposer; capacity/deadline/overflow detection is implemented | A3/A4/A8 | Add OS/JUCE lock and file-I/O interception or explicitly accept the traced boundaries; external product callbacks remain outside the guarantee |
