...
| Multisample/SFZ | DecentSampler zones plus disconnected Beat-owned C3A parsing, C3B resolution, C3C1 bounded immutable decoding, and C3C2 descriptor identity; canonical containment, `O_NOFOLLOW`, exact device/inode/size/time checks before and after decode, duplicate sharing, strict budgets, deterministic negative tests, and fail-closed unsupported platforms pass without upstream code or a dependency | C3A/C3B/C3C1/C3C2 | Add lifetime-safe control-thread publication, active-voice replacement policy, and deferred destruction; keep sequence metadata inactive until round-robin policy is separately approved |
