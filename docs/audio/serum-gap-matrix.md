...
| Dual serial warp stages | Implemented with independent stable fields and the bounded fold/pinch/mirror/shape set; D1A adds a deterministic 135-scenario comparative corpus without changing production DSP | B8/D1A | Current 2x is not spectrally closed; idealized 4x improves 133/135 cases but mirror-containing serial chains remain under-resolved. Measure a realtime-feasible higher-stop-band/8x candidate before proposing per-mode thresholds or production integration |
