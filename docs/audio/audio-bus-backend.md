...
The bottom main-editor panel exposes an accessible horizontal tab list for Master and every Audio Bus. The create-Bus button sits immediately before the final Bus tab (or after Master when no Buses exist), and the whole ribbon retains a continuous bottom rule. Users can switch tabs by pointer or Left/Right/Home/End keys and inspect the selected Bus in four stable columns: routed inputs, channel parameters, insert cards, and output/send routing. The panel reuses shared `SectionRibbon` headers, `RowItem` input rows, and the existing Button, Knob, Slider, TextInput, FloatingSelect, Toggle/MicroButton, icon, hover, and confirmation primitives; its remaining layout geometry is expressed through the existing design tokens rather than feature-local pixel constants. Bus input rows identify primary Track/Bus routes and send routes. Each input row uses the shared keyboard-accessible Aether Knob as a functional volume control: primary Track/Bus routes edit their source gain, while send rows edit the gain of that specific send. The Master input list is intentionally bus-only: it contains every enabled Bus whose primary output is Master, and updates when that route is changed or disabled. Full-width insert cards form an independently scrolling vertical stack and reuse Beat's selected power-icon control for enable/bypass. They expose reorder, removal, editable parameters, and the shared Aether curve-preview component; these curves are deterministic parameter-response illustrations, not measured spectrum or transfer telemetry. The output column owns rename, mono/stereo mode, cycle-safe primary output, metering, and cycle-safe sends (add, enable, level, pre/post-fader, and remove). Bus deletion remains confirmation-gated. Master uses the same Inputs, Parameters, and Inserts layout, omitting the redundant Output column; its fixed EQ and compressor remain editable, and the EQ automation editor remains available. The former Master EQ preset control is intentionally absent.
