...
The test demonstrates the key architectural requirement: interpolation of already-rendered base-rate sources is insufficient, so later production integration must render both A and B at the oversampled rate before multiplication. The stage owns only scalar biquad history and coefficients, performs no allocation, lock, file operation, lazy initialization, or container growth in `process`, and passes the focused callback-safety probe. Production output and the frozen render manifest remain unchanged pending that integration.
