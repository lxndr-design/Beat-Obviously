...
Aether Slot 1 is now connected to Beat's existing project audio-asset decoder and cache. The stable synth patch fields are `aether.sample.1.enabled`, `audioFileId`, `rootNote`, `level`, `pan`, and `route`; synth patch schema v2 migrates schema-v1 patches with the slot disabled and empty. The project Aether object carries its own versioned `sampleSlot1` record. A missing asset, empty ID, or unsupported future slot schema disables only Slot 1. Slots 2 and 3 remain reserved.
