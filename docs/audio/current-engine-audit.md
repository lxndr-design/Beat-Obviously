...
This closes callback heap-allocation detection for the exercised dense core path. It does not claim that arbitrary client-provided `onSegmentTriggered` or `onPositionChanged` callbacks are allocation-free, and it is not yet a runtime interposer for file operations or every OS/JUCE blocking primitive. The engine's own project-state boundary uses `ScopedTryLock`, but JUCE synthesiser internals retain their existing callback lock. Those remaining boundaries must be explicitly accepted or separately instrumented before the phase is described as having a universal forbidden-operation proof.
