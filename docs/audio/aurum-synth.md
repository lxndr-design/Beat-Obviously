...
- A bounded shared-modulation bridge from LFO 1, Envelope 1, Macro 1, Macro 2, velocity, keytrack, mod wheel, and pressure to master level/pan, six operator level/pan pairs, or Filter A/B cutoff
- Versioned user presets with tag search, favorites, reversible audition, identity-safe apply, and confirmed deletion
...
The frontend stores Aurum configuration under `instrument.aurum`. Native parsing maps the same object to `InstrumentDefinition::AurumConfig`; project persistence writes it back without flattening the matrices, operator harmonic spectra, response curves, filter modules, shared modulation routes, or macro values. Schema version 3 adds 16 normalized harmonic amplitudes per operator; version 4 adds normalized per-operator wavefold; version 5 adds dedicated pitch and phase ADSRs with bipolar semitone and degree depths; version 6 adds five-point velocity and keyboard gain-response curves; version 7 adds the operator-network quality mode; version 8 adds two output filters and Serial/Parallel routing; version 9 adds the 6 x 3 bipolar output-routing matrix; version 10 adds per-operator pan; version 11 adds the bounded shared-modulation bridge and eight neutral macro values; version 12 adds normalized operator level/pan and Filter A/B cutoff destinations; version 13 adds dedicated Filter A/B resonance and drive destinations. Version 11 retains only its master routes during migration, and version 12 retains only its established operator and cutoff routes, so new destinations remain neutral. Older patches migrate with a fundamental-only spectrum, zero wavefold, zero pitch/phase depth, flat response curves, centered operator pan, silent modulation, neutral macros, and 1x quality. Version 7 and earlier patches copy the instrument's shared filter into enabled Filter A, leave Filter B bypassed, and route the legacy `OUT` column into Filter A. Version 8 Serial patches route the legacy `OUT` column into Filter A; Parallel patches route it into both filters to preserve the v8 shared-input sound. New patches default to 2x quality.
...
The editor keeps the selected FM, RM, or OUT routing matrix visible while the left workspace switches between Main and OP 1 through OP 6. Main contains shared voice, unison, quality, dual-filter, Serial/Parallel routing, and modulation controls. The modulation table supports up to eight routes with shared source/destination labels, enable state, bipolar mode, and amount. Existing operator level/pan and filter-cutoff controls show compact shared status chips with active source and amount summaries. Each filter uses the shared Beat toggle, floating select, and slider components. The OUT matrix uses the shared bipolar knob component for Filter A, Filter B, and Direct columns; the selected operator page mirrors those three sends. Operator pages combine an engine-sampled live waveform scope, phase and wavefold shaping, tuning, level, and bipolar pan controls, plus a compact Amp/Pitch/Phase articulation selector over the operator's three ADSRs. Pitch and phase modes expose their bipolar depth beside the shared envelope controls. A shared Velocity/Key response graph exposes five fixed input positions with draggable, keyboard-adjustable gain points and Flat/Rise/Fall presets. Additive operators expose a drawable 16-bin spectrum with keyboard-adjustable bins and fundamental, odd, and saw presets.

The Presets control opens Beat's shared instrument-preset browser. User records retain versioned Aurum sound state and shared tag/favorite metadata. Search matches normalized name and tag tokens; audition never mutates the draft, applying a record preserves the destination instrument's identity and library placement, and deletion requires confirmation. Immutable Factory Bank v2 records add zero-neutral v13 performance routes while favorites remain separate per-user metadata; human listening sign-off and durable hashes remain roadmap items.
