...
Aurum is an instrument foundation, not a clone of another synthesizer. It does not yet include per-operator filter sends, arbitrary modulation curves, or a preset browser. Both filters currently process the complete operator-matrix output: Serial cascades enabled filters in A-then-B order, while Parallel processes the same matrix output through each enabled branch and averages active branches at unity-safe gain. Each operator's amplitude release governs its audible note-off tail; pitch and phase releases continue their articulation during that tail without extending an otherwise silent native voice. Velocity and MIDI-note position are evaluated once per voice against each operator's linearly interpolated five-point gain curves; flat curves are neutral. The hidden shared amplitude envelope does not reshape Aurum's operator envelopes. Additive rendering normalizes active partials and suppresses harmonics at or above Nyquist for the current operator frequency. Wavefold is an identity transform at zero and uses a bounded symmetric fold at positive depth.
