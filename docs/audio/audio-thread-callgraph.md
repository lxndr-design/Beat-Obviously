...
There is no new call-graph edge in C3F. The proposed boundary is: descriptor-safe decode and immutable spectral analysis on a worker/control side; manifest/hash/dimension validation and all FFT/window/resampler construction before publication; then fixed-capacity frame interpolation, phase propagation, inverse transform, overlap-add, and output resampling in the callback. File access, decoding, analysis, plan creation, allocation, worker signaling, container growth, repair, and ownership destruction are forbidden in render. This proposal remains blocked until the phase, stereo, resampling, latency, and hard-work policies receive specialist DSP sign-off.
