...
## Lumen Serum 2 behavior-reference benchmark — 2026-07-19

Lumen uses Xfer Records' public Serum 2 product page and web manual only to define a high-level behavior/capability comparison. Reviewed canonical pages are `https://www.xferrecords.com/products/serum-2`, `https://xferrecords.com/web-manual/serum-2/exploring-sound-design-in-serum`, `https://xferrecords.com/web-manual/serum-2/routing-an-oscillator-or-filter`, `https://xferrecords.com/web-manual/serum-2/exploring-serum`, `https://xferrecords.com/web-manual/serum-2/using-the-serum-keyboard`, and `https://support.xferrecords.com/article/58-how-to-upgrade-from-serum-1-to-serum-2`. Classification is reference-derived requirements only. No source code, preset, wavetable, sample, preview audio, artwork, branding, executable, service integration, or dependency was copied, adapted, imported, or executed.

Local destinations are `docs/audio/lumen-serum2-benchmark.md`, `docs/audio/lumen-serum2-capability-matrix.json`, and `docs/audio/lumen-architecture.md`. The Lumen identity/schema implementation is independently implemented Beat-owned code. Its v1 renderer adapter deliberately reuses Beat's existing Aether implementation as an internal starting point while preserving distinct `lumen-hybrid-synth` / `lumen` persistence identity. This entry does not claim a direct Serum 2 audio or CPU comparison because no locally installed reference renderer was available.

The Lumen v7 arpeggiator is independently implemented Beat-owned work using the existing JUCE MIDI container and Beat Sequencer timing boundary. No third-party source, preset, MIDI pattern, asset, executable, service, or new dependency was imported or executed. Local implementation boundaries are `backend/Source/Audio/Midi/LumenArpeggiator.h`, the Lumen-only route preparation/insertion in `backend/Source/Audio/AudioEngine.{h,cpp}`, the versioned fields in `TrackModel.h` and `SynthPatchContract.cpp`, and shared-component controls in `SynthEditor.solid.tsx`. It does not alter repository licensing or ownership and is not Vital-derived.

The Lumen v8 swing extension is likewise independently implemented Beat-owned work. It adds only a bounded alternating interval calculation, versioned persistence, shared-knob UI, and focused timing/migration tests within the same local boundaries. No upstream code, musical pattern, asset, executable, service, binary, or dependency was imported or executed.

The Lumen v9 key/scale extension is independently implemented Beat-owned work using fixed pitch-class masks and deterministic nearest-note arithmetic. It adds no musical content, preset, third-party scale library, external code, executable, service, binary, or dependency. Its implementation remains inside the existing Lumen arpeggiator, versioned Lumen schema/parser, shared Beat selector UI, and focused tests.

The Lumen v10 clip-sequencer foundation is independently implemented Beat-owned work. It uses a bounded monophonic 1-32-step relative-note contract, fixed callback storage, Beat's existing route MIDI boundary, and shared Beat UI components. Local implementation boundaries are `backend/Source/Audio/Midi/LumenClipSequencer.h`, the Lumen route preparation/insertion in `backend/Source/Audio/AudioEngine.{h,cpp}`, `TrackModel.h`, `SynthPatchContract.cpp`, `frontend/src/state/synthStore.ts`, and `SynthEditor.solid.tsx`. No third-party code, MIDI clip, preset, asset, executable, service, binary, or dependency was imported or executed; it is not Vital-derived.

The Lumen v11 single-clip polyphony upgrade is independently implemented Beat-owned work. It migrates v10 steps into a bounded schema-2 list of at most 64 relative notes, reuses Beat's existing piano roll through a pure adapter, and extends the fixed-capacity native sequencer with chord starts, independent note-offs, and safe same-pitch retrigger retirement. It does not add clip banks, recording, import, or third-party content. No upstream code, preset, MIDI file, asset, executable, service, binary, or dependency was imported, adapted, or executed; it is not Vital-derived.

## Aurum Sytrus behavior-reference benchmark — 2026-07-26

Aurum uses Image-Line's official public Sytrus manual at `https://www.image-line.com/fl-studio-learning/fl-studio-online-manual/html/plugins/Sytrus.htm` only to define a high-level feature-contract comparison for six-operator FM/RM/additive synthesis, matrix routing, operator articulation, filters, unison, effects, and patch workflow. No Image-Line source code, preset, oscillator data, sample, preview audio, artwork, branding, executable, service integration, or dependency was copied, adapted, imported, or executed. No locally installed Sytrus renderer was found, so no direct sound-quality, null-test, preset, or CPU comparison is claimed.

Local destinations are `docs/audio/aurum-sytrus-benchmark.md`, `docs/audio/aurum-sytrus-capability-matrix.json`, and `docs/audio/aurum-roadmap.md`. The audit records current Beat-owned behavior only and introduces no DSP, runtime dependency, binary, asset, preset, license, or ownership change.
...
| --- | --- | --- | --- |
| /Users/alexcheng/Documents/aether_future_bass_strings_preset.json | e7bc2d66bffb4cc1dc04b6973c52eac0150732fab8f6768b5625af1d9dd09016 | frontend/src/data/factory_demo_starter_bank_v4_acoustic_synth_guide.json / Benchmark - Future Bass Strings | user-supplied, structurally adapted |
| /Users/alexcheng/Documents/aether_progressive_house_strings_preset.json | 2d5874986920fca3852d041200b64392105a3dfedd6920dd87a815891dcb8d55 | frontend/src/data/factory_demo_starter_bank_v4_acoustic_synth_guide.json / Benchmark - Progressive House Strings | user-supplied, structurally adapted |

The adaptation converts normalized source values to the factory guide's percentage representation, converts phase degrees to cycle position, maps oscillator-wide vibrato to matched A/B fine-pitch routes, and preserves the authored static oscillator, envelope, LFO, filter, macro, performance, and effect values supported by Aether. The Future Bass source requests nine unison voices. The owner-approved Serum 1 closeout raises Aether's fixed per-oscillator capacity to 16, so the registered factory-guide preset now stores and renders the authored nine voices while remaining schema-idempotent. Dynamic routes targeting FX parameters, envelope time, LFO depth/smoothing, and other unavailable Aether modulation targets are not represented; their static target values remain present. No test was weakened to accept the conversion, and every changed render hash is explicitly reviewed in `aether-serum-benchmark.md`.

The 16-voice implementation is independently implemented Beat-owned code. It centralizes existing fixed-array sizes and clamps in `backend/Source/Audio/Wavetable/WavetableUnisonConfig.h` and `frontend/src/audio/aetherLimits.ts`, then applies the bound through existing Beat oscillator, persistence, IPC, preview, worklet, editor, and test paths. It imports no upstream source, preset, asset, binary, service, branding, or runtime dependency; no file is Vital-derived and no repository-wide licensing or ownership declaration changed.

The subsequent browser reference-render optimization in `frontend/src/audio/synthPreview.ts` is also independently implemented Beat-owned code. It replaces repeated hot-loop key/object/trigonometric work with bounded caches and stable configuration identity, changes no native DSP or persisted schema, and preserves every frozen output hash. No upstream source, dependency, binary, preset, asset, service, branding, license, or ownership declaration was introduced or changed.

The supplied documents name their four controls generically as Motion, Color, Shape, and Space. Beat's factory-preset audit rejects that shared generic display layout, so the structurally adapted product labels are `Pump / Brightness / Bite / Space` for Future Bass and `Drift / Warmth / Bloom / Hall` for Progressive House. Stable macro IDs, default values, and supported routes remain the recorded source adaptation; this naming refinement does not claim support for the omitted dynamic targets.

These are user-provided benchmark definitions, not Vital or other upstream factory presets. During the Serum 1 integration, the newer factory-guide records became the single registered definitions for both stable IDs; the older `frontend/src/data/aether_benchmark_strings_bank.json` adaptation remains retained as provenance evidence but is no longer loaded into the factory library. This prevents duplicate stable IDs. The resulting six frontend C4 hashes intentionally supersede the older adaptation only after their data differences, deterministic repetition, audibility metrics, and unchanged standalone audition hashes were recorded in `aether-serum-benchmark.md`. No acquired repository, branding, service endpoint, binary, executable content, runtime dependency, repository-wide license, or ownership declaration was imported or changed. Local implementation files are `frontend/src/data/factory_demo_starter_bank_v4_acoustic_synth_guide.json`, `frontend/src/data/aether_benchmark_strings_bank.json`, `frontend/src/state/synthStore.ts`, `scripts/verify-synth-roundtrip.mjs`, two unrelated Solid setter compatibility fixes exposed by the full gate, and these four audit documents; none is Vital-derived.
...
## Lumen v2 three-slot wavetable foundation provenance

The fixed A/B/C identity contract, deterministic v1 migration, Lumen-only Slot C prepared state, routing, source sends, browser rendering, editor constraints, and focused tests are independently implemented Beat-owned work using existing Beat wavetable, voice, UI, and serialization primitives. No external repository, source, preset, wavetable, sample, binary, service, endpoint, or new dependency was consulted, imported, executed, or added. Aether code paths and defaults remain the regression oracle; Slot C is unreachable unless the engine identity is Lumen and its explicit parameter is enabled.

Changed implementation files are `frontend/src/state/synthStore.ts`, `frontend/src/features/Synth/OscillatorPanel/OscillatorPanel.solid.tsx`, `backend/Source/Audio/Parameters/ParameterIds.h`, `backend/Source/Audio/Parameters/SynthPatchContract.cpp`, `backend/Source/Audio/TrackModel.h`, `backend/Source/Audio/AudioEngine.cpp`, `backend/Source/Audio/InstrumentVoice.h`, `backend/Source/Audio/InstrumentVoice.cpp`, `backend/Source/Audio/Oscillator/VoiceAetherCache.h`, `backend/Tests/BackendStress.cpp`, `scripts/verify-synth-roundtrip.mjs`, `scripts/verify-lumen-foundation.mjs`, and the Lumen benchmark documents. No repository-wide license or ownership declaration changed, and no file is Vital-derived.

## Aurum transactional history and Aether-to-Lumen clone provenance

The bounded Aurum undo/redo history and the explicit Aether-to-Lumen clone are independently implemented Beat-owned workflow code using existing Beat instrument snapshots, editor components, patch normalization, and library persistence. The Lumen conversion does not inspect or translate Serum data: it copies Beat's normalized Aether state into Beat's independently versioned Lumen schema, preserves the source instrument and derivation lineage, and forces Lumen-only Source C, Arpeggiator, and Clip features inactive. No external source, preset, patch, audio, artwork, executable, service, dependency, or binary was inspected, copied, translated, adapted, imported, or added.

Local implementation files are `frontend/src/features/Aurum/aurumHistory.ts`, `frontend/src/features/Aurum/AurumEditor.solid.tsx`, `frontend/src/state/synthStore.ts`, `frontend/src/state/store.ts`, `frontend/src/features/InstrumentLibrary/InstrumentLibraryPanel.solid.tsx`, `frontend/src/features/Synth/SynthEditor/SynthEditor.solid.tsx`, `scripts/verify-aurum.mjs`, `scripts/verify-synth-roundtrip.mjs`, `scripts/verify-lumen-foundation.mjs`, and the Aurum/Lumen comparison documents. The store adjustment only snapshots the plain current value of an existing Immer draft before placing a deleted instrument on Beat's undo stack. No DSP renderer, Aether default, repository-wide license, or ownership declaration changed, and no file is Vital-derived.

## Vital source-architecture benchmark review

On 2026-07-28, the pinned external Vital GPLv3 snapshot `636ca0ef517a4db087a6a08a6a8a5e704e21f836` was inspected for architecture comparison against Lumen. The review covered packed SIMD values, enabled-processor traversal, modulation rate selection and remapping, active modulation-source pruning, producer/filter/effect enablement, oscillator unison/spectral-buffer behavior, and wavetable publication. The source remained in `/Users/alexcheng/beat-aether-references/vital`; it was not built, executed, linked, copied, modified, imported, or added as a Beat dependency. No Vital preset, wavetable, sample, coefficient table, artwork, branding, service, or binary was used.

The resulting `docs/audio/lumen-vital-source-audit.md` is documentation and requirements analysis only. It recommends independently designed Beat implementations of general techniques including sparse execution, control/audio-rate classification, immutable prepared buffers, active-lane masks, lookup tables, and SIMD. Any future copied, translated, or closely adapted Vital implementation remains subject to the ledger's existing file-level GPLv3 provenance, notices, verifier, and private-build/no-distribution gates. This review changes no DSP renderer, project schema, repository-wide license, or ownership declaration, and no Beat production file is Vital-derived.

## Prepared modulation and active warp-lane provenance

The cached sparse target lists, bounded modulation input frame, legacy-order evaluator, disabled-plan fast path, and active/history-bearing runtime-warp lane predicate are independently implemented Beat-owned work from Beat's existing modulation fields, runtime warp state, and the requirements recorded by the source-architecture benchmark review. No Vital source, implementation structure, expression, coefficient, lookup table, preset, wavetable, sample, binary, branding, service, dependency, or build instruction was copied, translated, structurally adapted, imported, linked, or added to Beat.

Changed production files are `backend/Source/Audio/Modulation/DynamicModulation.h`, `backend/Source/Audio/Oscillator/AetherTableStackRenderer.h`, `backend/Source/Audio/InstrumentVoice.h`, and `backend/Source/Audio/InstrumentVoice.cpp`. Focused coverage is in `backend/Tests/BackendStress.cpp`; the audit, benchmark, integration record, and verifier were updated. No project or instrument schema, parameter identifier, preset field, runtime-warp equation, external dependency, repository-wide license, or ownership declaration changed. No Beat production file is Vital-derived.
