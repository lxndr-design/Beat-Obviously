...
- `[~]` Deeper envelope editing.
  - Done: Env 1/2 visual summary cards show ADSR shape, loop mode, timing, sustain, curve choices, and routed assignment labels; direct ADSR handles now edit attack, decay/sustain, and release against the real synth parameters.
  - Remaining: richer direct curve-shape editing beyond the current curve button set.
  - Proof: interaction tests for handle movement, patch roundtrip, browser preview, and native render stress; true browser fixture coverage now exercises Env 1 handle dragging through real pointer events and records changed attack/decay/sustain/release patch values.
