...
- `[~]` Add proper voice stealing, mono/legato, glide, pitch bend, mod wheel, velocity, keytracking. Filter keytracking, keytrack modulation source routing, mod wheel routing, runtime pitch bend, linked-note glide timing, explicit `maxVoices` caps with native note stealing, mono voice caps, legato voice retune behavior, Aether patch/editor glide editing, dedicated voices/mono/legato/glide Performance controls, and initial velocity modulation now exist across patch/browser/native paths; realtime expression feedback remains.
