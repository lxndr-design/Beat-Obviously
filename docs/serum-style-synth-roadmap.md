...
- `[~]` Add LFO 2+, tempo sync, one-shot mode, smoothing, phase/random controls. LFO 2 now exists as a patch/native/browser modulation source, and normalized phase offsets are honored for note retrigger; tempo-sync behavior, one-shot mode, smoothing, and random phase controls still need completion.
