...
- `[~]` Add macro semantics: named macros, ranges, curves, and visible assignments. Durable macro metadata now supports labels, min/max ranges, response curves, range-aware visible assignment summaries, browser static-route math, native macro range/curve parsing, first-class voice macro state, macro route storage in dynamic targets, and `macro.1`-`macro.4` realtime/note automation with backend no-leak stress; browser preview parity, explicit project/document roundtrip fixtures, deeper macro assignment UX, and per-target range displays remain.
