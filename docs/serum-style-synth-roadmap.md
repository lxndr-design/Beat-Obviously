...
- `[~]` Add macro semantics: named macros, ranges, curves, and visible assignments. Durable macro metadata now supports labels, min/max ranges, response curves, range-aware visible assignment summaries, and matching browser/native static-route math; deeper macro assignment UX and per-target range displays remain.
