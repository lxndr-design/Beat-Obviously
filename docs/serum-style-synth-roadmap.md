...
- `[~]` Dynamic modulation routes exist for LFO 1, LFO 2, Env 1, Env 2, and velocity sources to oscillator, filter, amp, and unison targets.
...
- `[~]` Add richer envelopes: Env 1 curve shapes now roundtrip through the patch contract, Solid editor, browser preview, native parser, native render shaping, IPC import, and native persistence; Env 2 now has independent modulation ADSR semantics in browser/native render paths; looping envelopes and deeper envelope editing remain.
...
- `[~]` Add proper voice stealing, mono/legato, glide, pitch bend, mod wheel, velocity, keytracking. Filter keytracking and initial velocity modulation now exist across patch/browser/native paths; voice stealing, mono/legato, glide behavior, pitch bend, mod wheel, and deeper velocity/expression UX remain.
