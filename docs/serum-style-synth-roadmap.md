...
- `[~]` Extract oscillator, envelope, LFO, modulation, filter, and voice-allocation logic out of `InstrumentVoice`. Native envelope curve shaping plus attack/decay loop rendering and LFO shape/routing helpers now live in shared native helpers with direct stress coverage; oscillator, modulation, filter, and allocation extraction remain.
