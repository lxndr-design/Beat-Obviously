...
This roadmap replaces the older Serum-style checklist. The focused v1 DAW completion plan lives in `docs/daw-comprehensiveness-roadmap.md`; the focused remaining Aether burn-down list lives in `docs/aether-completion-checklist.md`; current low-level audio-engine decisions and verification details live in `docs/audio-engine.md`; folder-structure migration guidance lives in `docs/project-structure.md`; stable synth IDs live in `docs/synth-parameter-contract.md`.
...
> Audit gate (2026-07-11): the Serum-class foundation audit in `docs/audio/current-engine-audit.md` found that pitch currently constrains wavetable timbral position because timbral frames and harmonic mip levels share one axis. It also found no callback-wide real-time-safety proof, no Beat-owned deterministic victim selector, no audible-path DC blocker, and no offline-only HQ DSP mode. The aggregate native and non-native release gates are currently red on documented pre-existing failures. Treat the readiness summary below as product breadth, not Milestone A release evidence, until those audit gates are cleared.
...
- Effects/automation: `65%`
  ...
  - Next steps: bus routing UI, named effect preset library, automation selection browser tests, arrangement-view automation lanes, arbitrary multi-point lane editing, and unified project/instrument automation UX.
- UI/UX coherence: `54%`
...
- `[~]` Project/segment/note/track automation are native. Piano-roll note lanes, first Segment Editor Aether segment lanes, and first Track Details Aether track lanes now have visible editing surfaces; arrangement track lanes and broader project/instrument automation UX remain early.
...
- `[~]` Extract oscillator, envelope, LFO, modulation, filter, wavetable-cache, wavetable-oscillator-bank rendering, wavetable-unison planning, Aether table-stack rendering, Aether voice cache math, voice utility math, render-stat types/ownership/per-block assembly, note-automation inboxing/types/runtime state, realtime ramp state/parameter IDs/value policy/param application/active-index bookkeeping/reset helpers, and voice-allocation logic out of `InstrumentVoice`. Native envelope curve shaping, attack/decay loop rendering, LFO shape/routing, dynamic modulation source routing/target offset math/activity snapshots/source-aware render planning, built-in oscillator/polyBLEP, filter math/state ownership, oversampled drive-stage helpers, shared wavetable voice-cache ownership, reusable wavetable oscillator-bank setup/render helpers, reusable wavetable unison plan math, reusable Aether table-stack render/mix helpers, reusable Aether pan/pitch cache helpers, reusable voice math/noise/pan/pitch/clamp helpers, shared render-work stat types/counters/per-block work helpers, realtime note-automation handoff storage, context types, and runtime state, reusable realtime ramp state and active-ramp bookkeeping/reset helpers, reusable voice realtime parameter ID mapping/value clamping/param application, and voice-allocation policy now live in shared native helpers with direct stress coverage; deeper modulation UI cleanup remains.
- `[x]` Add richer envelopes: Env 1 and Env 2 curve shapes now roundtrip through the patch contract, Solid editor, browser preview, native parser, native render shaping, and verifier/stress coverage; Env 1 and Env 2 loop modes now exist across Solid/browser/native/stress, and the editor now shows visual ADSR cards with loop/curve/assignment summaries, direct ADSR drag handles, and direct rail curve controls for attack, decay, and release.
...
- `[~]` Add macro semantics: named macros, ranges, curves, and visible assignments. Durable macro metadata now supports labels, min/max ranges, response curves, range-aware visible assignment summaries, browser static-route math, browser live-preview parity for macro route edits/disabled routes, native macro range/curve parsing, first-class voice macro state, macro route storage in dynamic targets, `macro.1`-`macro.4` realtime/note automation with backend no-leak stress, and explicit project/document roundtrip fixtures; deeper macro assignment UX and per-target range displays remain.
...
- `[x]` Add advanced warp modes with bounded CPU cost. Aether now exposes `shape`, `fold`, `pinch`, and `mirror` table-generation modes through Solid controls, browser/worklet preview, native table generation, IPC/persistence, patch parsing, and verifier/stress coverage; opt-in `aether.runtimeWarp` adds a separate runtime nonlinear Aether stack stage before filter drive with bounded browser/native proof.
- `[x]` Add oversampling strategy for nonlinear stages. Route saturator/distortion, Aether runtime warp, and Aether filter-drive nonlinear stages now use bounded 2x midpoint oversampling with block-continuous state; route/filter drive keep explicit timing counters, and runtime warp is proven through output-difference stress coverage.
- `[x]` Add proper voice stealing, mono/legato, glide, pitch bend, mod wheel, velocity, keytracking. Filter keytracking, keytrack modulation source routing, mod wheel routing, runtime pitch bend, linked-note glide timing, explicit `maxVoices` caps with native note stealing, mono voice caps, legato voice retune behavior, Aether patch/editor glide editing, dedicated voices/mono/legato/glide Performance controls, velocity modulation, browser timeline expression feedback, browser Web MIDI expression feedback, and native JUCE MIDI expression feedback now exist across patch/browser/native paths.
