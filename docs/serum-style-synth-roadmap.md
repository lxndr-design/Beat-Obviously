...
- `[~]` Extract oscillator, envelope, LFO, modulation, filter, wavetable-cache, wavetable-unison planning, voice utility math, render-stat ownership, note-automation inboxing, realtime ramp state, and voice-allocation logic out of `InstrumentVoice`. Native envelope curve shaping, attack/decay loop rendering, LFO shape/routing, dynamic modulation source routing/target offset math/activity snapshots/source-aware render planning, built-in oscillator/polyBLEP, filter math/state ownership, oversampled drive-stage helpers, shared wavetable voice-cache ownership, reusable wavetable unison plan math, reusable voice math/noise/pan/pitch/clamp helpers, shared render-work stat counters, realtime note-automation handoff storage, reusable realtime ramp state, and voice-allocation policy now live in shared native helpers with direct stress coverage; deeper modulation UI cleanup remains.
