...
- `[x]` Add advanced warp modes with bounded CPU cost. Aether now exposes `shape`, `fold`, `pinch`, and `mirror` table-generation modes through Solid controls, browser/worklet preview, native table generation, IPC/persistence, patch parsing, and verifier/stress coverage.
- `[x]` Add oversampling strategy for nonlinear stages. Route saturator/distortion and Aether filter-drive nonlinear stages now use shared bounded 2x midpoint oversampling with block-continuous state, explicit work-cost helpers, timing counters, and native stress coverage; Aether warp modes remain bounded at wavetable-generation time rather than per-sample runtime.
