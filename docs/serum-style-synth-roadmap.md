...
- `[~]` Extract oscillator, envelope, LFO, modulation, filter, wavetable-cache, and voice-allocation logic out of `InstrumentVoice`. Native envelope curve shaping, attack/decay loop rendering, LFO shape/routing, dynamic modulation source routing/target offset math/activity snapshots/source-aware render planning, built-in oscillator/polyBLEP, filter math/state ownership, oversampled drive-stage helpers, shared wavetable voice-cache ownership, and voice-allocation policy now live in shared native helpers with direct stress coverage; deeper modulation UI cleanup remains.
