...
- `[~]` Extract oscillator, envelope, LFO, modulation, filter, and voice-allocation logic out of `InstrumentVoice`. Native envelope curve shaping plus attack/decay loop rendering now live in a shared envelope helper with direct stress coverage; oscillator, LFO, modulation, filter, and allocation extraction remain.
