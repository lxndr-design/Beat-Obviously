...
- `[~]` Add LFO 2+, tempo sync, one-shot mode, smoothing, phase/random controls. LFO 2 now exists as a patch/native/browser modulation source, normalized phase offsets are honored for note retrigger, and retrigger/one-shot can be toggled per LFO; tempo-sync behavior, LFO smoothing, and LFO random phase controls still need completion. Oscillator phase/random-phase controls are implemented separately.
