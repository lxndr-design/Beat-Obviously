...
- `[~]` Dynamic modulation routes exist for LFO 1, LFO 2, and envelope sources to oscillator, filter, amp, and unison targets.
...
- `[~]` Add LFO 2+, tempo sync, one-shot mode, smoothing, phase/random controls. LFO 2 now exists as a patch/native/browser modulation source, normalized phase offsets are honored for note retrigger, and retrigger can be toggled per LFO; tempo-sync behavior, one-shot mode, LFO smoothing, and LFO random phase controls still need completion. Oscillator phase/random-phase controls are implemented separately.
