...
- `[~]` Add wavetable import, morphing, drawing, FFT/resynthesis research lane, and frame normalization rules. Audio-file wavemap import, deterministic frame normalization, seeded non-AI evolution, per-frame `skew` harmonic-bias editing, per-frame `formant` resonant-band editing, per-frame `notch` harmonic-carve editing, 16-bin harmonic partial drawing, and browser/native `linear` versus `smooth` wavemap interpolation now exist; deeper waveform drawing, FFT analysis, and morphing controls remain.
