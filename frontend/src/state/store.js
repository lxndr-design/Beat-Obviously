...
let soloAutoMutedTrackIds = new Set();
export const useProjectStore = create()(temporal(immer((set) => ({
    ...
    reorderTracks: (orderedIds) => set((s) => {
    ...
    removeSegment: (segmentId) => set((s) => {
        for (const track of s.project.tracks) {
            track.segments = track.segments.filter((seg) => seg.id !== segmentId);
        }
    }),
    moveSegment: (segmentId, toTrackId, toStartBeat) => set((s) => {
        let seg;
        for (const t of s.project.tracks) {
    ...
    updateSegment: (segmentId, patch) => set((s) => {
        for (const t of s.project.tracks) {
    ...
    setSegmentRepeats: (segmentId, repeats) => set((s) => {
        for (const t of s.project.tracks) {
            const seg = t.segments.find((x) => x.id === segmentId);
            if (seg) {
...
export const canUndo = () => useProjectStore.temporal.getState().pastStates.length > 0;
export const canRedo = () => useProjectStore.temporal.getState().futureStates.length > 0;
...
export const useUiStore = create()((set) => ({
    ...
    selectSegment: (id, additive) => set((s) => ({
    ...
    clearSelection: () => set({ selectedTrackIds: [], selectedSegmentIds: [] }),
    openEditor: (e) => set((s) => ({
...
export const useInstrumentStore = create()(immer((set, get) => ({
    instruments: [],
    addInstrument: (patch) => {
    ...
    removeInstrument: (id) => set((s) => {
        ...
        if (!target || !target.userCreated)
            return;
        s.instruments = s.instruments.filter((i) => i.id !== id);
    }),
    updateInstrument: (id, patch) => set((s) => {
    ...
    seedSystemInstruments: () => {
        ...
        if (existing.some((i) => !i.userCreated))
            return;
        const sampler = (name, sampleUrl, knobs = { cutoff: 0.7, resonance: 0.15, drive: 0.1, color: 0.5 }) => ({
            id: nanoid(),
            name,
            kind: "sampler",
    ...
    mergeInstruments: (aId, bId) => {
        ...
        const merged = {
            id: nanoid(),
            name: `${a.name} × ${b.name}`,
            kind: a.kind === b.kind ? a.kind : a.kind === "synth" ? "hybrid" : "hybrid",
            envelope: {
...
export const useAudioFileStore = create()(immer((set) => ({
    files: [],
    addFile: (file) => set((s) => {
        if (s.files.some((f) => f.id === file.id))
            return;
        s.files.push(file);
    }),
    removeFile: (id) => set((s) => {
...
function restoreSoloAutoMutes(tracks) {
    for (const t of tracks) {
        if (soloAutoMutedTrackIds.has(t.id))
            t.mute = false;
        t.solo = false;
    }
    soloAutoMutedTrackIds = new Set();
 ...
 */
function computeLayerForInsertion(track, seg) {
    ...
    for (const existing of track.segments) {
        ...
        if (overlaps)
            maxLayer = Math.max(maxLayer, existing.layer + 1);
    }
    return maxLayer;
}
