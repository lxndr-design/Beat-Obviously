...
import type { DrumRow, Id, Instrument, MidiNote } from "./types";
...
export const useComponentStore = create<ComponentSlice>()(
  immer((set) => ({
    ...
    seedDefaultDrumLoops: (instruments) =>
      set((s) => {
        if (s.components.some((component) => component.factory && component.kind === "drum")) return;
...
function makeFactoryDrumLoops(instruments: Instrument[]): DrumComponent[] {
  ...
  const loop = (
    name: string,
    rows: Array<{ instrument: Instrument | undefined; name: string; steps: number[] }>,
    speed: 1 | 2 | 4 | 8 = 1,
  ): DrumComponent => ({
    id: nanoid(),
    kind: "drum",
