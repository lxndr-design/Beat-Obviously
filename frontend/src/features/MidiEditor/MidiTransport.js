 ...
 */
export function MidiTransport({ notes, lengthBeats, bpm, instrument, hotkeyScopeId }) {
    ...
    function scheduleNote(n, atTimeS, durS, vel) {
        const ctx = getCtx();
    ...
    return (_jsxs("div", { style: { display: "inline-flex", gap: 8 }, children: [_jsx(HoverInfo, { content: "Restart", children: _jsx(Button, { iconOnly: true, size: "xs", onClick: restart, "aria-label": "Restart", children: _jsx(Icon, { name: "ph:skip-back-fill", size: 16, decorative: true }) }) }), _jsx(HoverInfo, { content: playing ? "Pause" : "Play", children: _jsx(Button, { iconOnly: true, size: "xs", variant: playing ? "primary" : "default", onClick: playing ? pause : play, "aria-label": playing ? "Pause" : "Play", children: _jsx(Icon, { name: playing ? "ph:pause-fill" : "ph:play-fill", size: 16, decorative: true }) }) })] }));
}
