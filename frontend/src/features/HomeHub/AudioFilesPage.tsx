...
export function AudioFilesPage() {
  ...
  const updateFile = useAudioFileStore((s) => s.updateFile);
  ...
  const [previewDirection, setPreviewDirection] = useState<PreviewDirection>("forward");
  ...
  const progressRef = useRef<{ startTime: number; duration: number; direction: PreviewDirection; loop: boolean }>({
    startTime: 0,
    duration: 1,
    direction: "forward",
    loop: false,
  ...
  useEffect(() => {
    setSelectedIds((current) => new Set([...current].filter((id) => files.some((file) => file.id === id))));
    if (activeId && !files.some((file) => file.id === activeId)) setActiveId(null);
  }, [activeId, files]);
  ...
  useEffect(() => {
    ...
    const loadBrowserAnalysis = () => {
      ...
      return loadAudioBuffer(ctx, bufferCacheRef.current, previewFile)
        .then((buffer) => {
          if (!cancelled) setWaveformAnalysis(analyzeAudioBuffer(buffer, 128));
        })
        .finally(() => {
          if (!previewRef.current || previewRef.current.ctx !== ctx) {
    ...
    const loadNativeAnalysis = async () => {
      const response = await send({ kind: "audio.waveform", path: previewFile.path, bucketCount: 128 });
      if (!response.waveform) throw new Error(response.error ?? "Waveform unavailable.");
      if (!cancelled) setWaveformAnalysis(nativeWaveformAnalysis(previewFile, response.waveform));
    };

    void (isNative() && previewFile.path && !previewFile.path.startsWith("data:")
  ...
  async function playPreview(
    ...
    try {
      ...
      const sourceBuffer = await loadAudioBuffer(ctx, bufferCacheRef.current, file);
  ...
  function toggleReversePlayback() {
    if (!previewFile) return;
    const currentlyReversing = playingId === previewFile.id && previewDirection === "reverse";
    ...
    void playPreview(previewFile, nextDirection, { restart: true, progress: handoffProgress });
  }

  function cycleSpeed() {
    const next = previewSpeed === 1 ? 2 : previewSpeed === 2 ? 3 : 1;
    setPreviewSpeed(next);
    if (previewFile && playingId === previewFile.id) {
      void playPreview(previewFile, progressRef.current.direction, { restart: true, speed: next, progress: previewProgress });
    }
  }
  ...
  function toggleLoop() {
    ...
    setLoopPreview(next);
    if (previewFile && playingId === previewFile.id) {
      void playPreview(previewFile, progressRef.current.direction, { restart: true, loop: next, progress: previewProgress });
    }
  }
  ...
  function scrubTo(progress: number) {
    ...
    if (previewFile && playingId === previewFile.id) {
      void playPreview(previewFile, progressRef.current.direction, { restart: true, progress: next });
    }
  }
  ...
  return (
    <AssetPageShell
      ...
      browser={
        ...
        <div className={`${styles.table} ${selectMode ? styles.tableSelectMode : ""}`} role="table" aria-label="Audio files">
          ...
          <div className={styles.rows}>
            {sortedFiles.length === 0 ? (
              ...
              return (
                <div
                  key={file.id}
                  className={`${styles.row} ${active ? styles.rowActive : ""} ${selected ? styles.rowSelected : ""}`}
                  onClick={(event) => {
                    setActiveId(file.id);
                  ...
                  {selectMode && (
                    <span className={styles.checkCell}>
                      <span className={`${styles.checkbox} ${selected ? styles.checkboxChecked : ""}`}>
                        {selected && <Icon name="ph:check" size={12} decorative />}
                      </span>
                    </span>
                  )}
        ...
        {multipleSelected ? (
          <div className={styles.previewBody}>
            <div className={styles.waveformStage}>
              ...
              <div className={styles.multipleSelected}>Multiple files selected</div>
            </div>
            <div className={styles.previewFileName}>–</div>
          ...
          <div className={styles.previewBody}>
            ...
            <div className={styles.previewControls}>
              <HoverInfo content="Play from start">
                <Button iconOnly onClick={() => void playPreview(activeFile, "forward", { restart: true, progress: 0 })} aria-label="Play from start">
                  <Icon name="ph:skip-back" size={14} decorative />
                </Button>
              </HoverInfo>
              <HoverInfo content="Play / pause">
                <Button iconOnly selected={playingId === activeFile.id} onClick={() => void playPreview(activeFile, "forward", { progress: previewProgress >= 1 ? 0 : previewProgress })} aria-label="Play or pause">
                  <Icon name={playingId === activeFile.id ? "ph:pause-fill" : "ph:play-fill"} size={14} decorative />
                </Button>
              </HoverInfo>
              ...
              <HoverInfo content="Play backwards">
                <Button iconOnly selected={playingId === activeFile.id && previewDirection === "reverse"} onClick={toggleReversePlayback} aria-label="Play backwards">
            ...
            <dl className={styles.details}>
              ...
              <Info label="File Size" value={formatFileSize(activeFile.sizeBytes)} />
              <Info label="Used In" value={formatUsage(activeFile.id, tracks)} />
              <Info label="Instrument" value={formatInstrumentUsage(activeFile, instruments)} />
              <Info label="Library Path" value={activeFile.path} />
...
function ScopeOverlay({ analysis }: { analysis: WaveformAnalysis | null }) {
  return (
    <>
      <span className={`${styles.waveformScopeLabel} ${styles.waveformDbTop}`}>{formatDb(analysis?.leftDb)}</span>
      <span className={`${styles.waveformScopeLabel} ${styles.waveformDbBottom}`}>{formatDb(analysis?.rightDb)}</span>
      <span className={`${styles.waveformLeftTick} ${styles.waveformLeftTickUpper}`} />
      <span className={`${styles.waveformLeftTick} ${styles.waveformLeftTickLower}`} />
      <span className={`${styles.waveformChannelLabel} ${styles.waveformChannelLeft}`}>L</span>
      <span className={`${styles.waveformChannelLabel} ${styles.waveformChannelRight}`}>R</span>
    </>
  );
}

function WaveformPreview({ analysis }: { analysis: WaveformAnalysis | null }) {
  if (!analysis) return <div className={styles.waveformLoading} />;
...
function SortHeader({ label, sortKey, current, onSort }: SortHeaderProps) {
  ...
  return (
    <button type="button" className={`${styles.sortHeader} ${active ? styles.sortHeaderActive : ""}`} onClick={() => onSort(sortKey)}>
      <span>{label}</span>
