...
export function InstrumentsPage() {
  ...
  const [samplePreviewIndex, setSamplePreviewIndex] = useState<Record<string, number>>({});
  ...
  const [renderState, setRenderState] = useState<{
    ...
    loading: boolean;
    error?: string;
  }>({ waveform: null, analysis: null, loading: false });
  const previewRef = useRef<{ source: AudioBufferSourceNode; gain: GainNode } | null>(null);
  const previewContextRef = useRef<AudioContext | null>(null);
  const nativePreviewBufferRef = useRef<Map<string, AudioBuffer>>(new Map());
  const sampleCursorRef = useRef<Map<string, number>>(new Map());
  const progressFrameRef = useRef<number | null>(null);
  const playbackBarRef = useRef<HTMLDivElement | null>(null);
  const progressRef = useRef<{ startTime: number; duration: number; loop: boolean }>({
    startTime: 0,
  ...
  }, [activeInstrument, activeSampleUrl]);
  ...
  function onPlaybackBarPointerDown(event: ReactPointerEvent<HTMLDivElement>) {
  ...
  function onPlaybackBarPointerMove(event: ReactPointerEvent<HTMLDivElement>) {
    if (!scrubbing || !activeInstrument) return;
    const progress = progressFromPointer(event.clientX);
    setPreviewProgress(progress);
  }

  function onPlaybackBarPointerUp(event: ReactPointerEvent<HTMLDivElement>) {
      ...
      preview={
        ...
        {activeInstrument ? (
          <div className={styles.previewBody}>
            ...
            <div
              ref={playbackBarRef}
              className={`${styles.playbackBar} ${scrubbing ? styles.playbackBarScrubbing : ""}`}
...
function InstrumentWaveform({ waveform }: { waveform: AudioWaveformSummary | null }) {
  if (!waveform || waveform.left.upper.length === 0) return null;
  const left = waveformPath(channelEnvelope(waveform.left), 50, -42);
  const right = waveformPath(channelEnvelope(waveform.right), 50, 42);
  return (
    <svg className={styles.waveformSvg} viewBox="0 0 100 100" preserveAspectRatio="none" aria-hidden="true">
      <polygon className={styles.waveformFill} points={left.fill} />
      <polyline className={styles.waveformTrace} points={left.trace} />
      <polygon className={styles.waveformFill} points={right.fill} />
      <polyline className={styles.waveformTrace} points={right.trace} />
    </svg>
  );
}
...
function SampleStructure({ instrument, activeSampleUrl }: { instrument: Instrument; activeSampleUrl?: string }) {
  ...
  const [draftOrder, setDraftOrder] = useState(() => sampleOrder(instrument));
  const [draftVariables, setDraftVariables] = useState<SampleVariable[]>(() => inferSampleVariables(instrument));
  const [draftGrouping, setDraftGrouping] = useState<SampleGrouping>(() => primarySampleGrouping(instrument));
  const [draftZoneEdits, setDraftZoneEdits] = useState<Record<string, Partial<NonNullable<Instrument["sampleMap"]>[number]>>>({});
  const [editingValue, setEditingValue] = useState<{ path: string; value: string } | null>(null);
  ...
  }, [instrument.id, instrument.sampleUrl, instrument.sampleUrls, instrument.sampleMap]);

  const draftInstrument = useMemo(
    () => materializeSampleDraft(instrument, draftOrder, draftVariables, draftZoneEdits),
    [draftOrder, draftVariables, draftZoneEdits, instrument],
  );
  const activeGrouping = draftVariables.includes(draftGrouping as SampleVariable) ? draftGrouping : draftVariables[0] ?? "single";
  const groups = sampleStructureGroups(draftInstrument, activeGrouping);
  if (groups.length === 0) return null;
  const structure = sampleStructureLabels(draftInstrument, draftVariables);
  const originalVariables = inferSampleVariables(instrument);
  const originalDraftInstrument = materializeSampleDraft(instrument, sampleOrder(instrument), originalVariables);
  const pendingPatch = editingValue ? parseSampleValueEdit(editingValue.value, activeGrouping) : null;
  const pendingZone = editingValue
    ? draftInstrument.sampleMap?.find((zone) => zone.path === editingValue.path)
    : undefined;
  const effectiveZoneEdits = editingValue && pendingPatch && samplePatchChangesZone(pendingZone, pendingPatch)
    ? {
        ...draftZoneEdits,
        [editingValue.path]: {
    ...
    : draftZoneEdits;
  const effectiveDraftInstrument = materializeSampleDraft(instrument, draftOrder, draftVariables, effectiveZoneEdits);
  const dirty = sampleDraftSignature(effectiveDraftInstrument) !== sampleDraftSignature(originalDraftInstrument);

  function saveDraft() {
    const committedEdits = committedZoneEdits();
    const next = materializeSampleDraft(instrument, draftOrder, draftVariables, committedEdits);
    updateInstrument(instrument.id, {
      ...
      source: instrument.source ? { ...instrument.source, edited: true } : instrument.source,
    });
  }
  ...
  function commitEditedValue() {
    ...
    const patch = parseSampleValueEdit(editingValue.value, activeGrouping);
    const currentZone = draftInstrument.sampleMap?.find((zone) => zone.path === editingValue.path);
    if (patch && samplePatchChangesZone(currentZone, patch)) {
      setDraftZoneEdits((current) => ({
        ...current,
        [editingValue.path]: {
          ...(current[editingValue.path] ?? {}),
          ...patch,
        },
      }));
    }
  ...
  function committedZoneEdits() {
    if (!editingValue) return draftZoneEdits;
    const patch = parseSampleValueEdit(editingValue.value, activeGrouping);
  ...
  return (
    <section className={styles.sampleStructure} aria-label="Associated audio files">
      <div className={styles.sampleStructureHeader}>
        <span>Associated Audio Files</span>
        <strong>{structure.join(" / ")}</strong>
      </div>
      <div className={styles.sampleGrouping} aria-label="Sample variables">
        {draftVariables.length === 0 ? (
          <span className={styles.sampleGroupingEmpty}>No variables</span>
          ...
          <button
            key={variable}
            type="button"
            className={variable === activeGrouping ? styles.sampleGroupingActive : ""}
        ...
        <button className={styles.sampleAddVariable} type="button" onClick={addVariable} disabled={draftVariables.length >= SAMPLE_VARIABLE_OPTIONS.length}>
          + Variable
        </button>
      </div>
      {groups.map((group) => (
        <div key={group.title} className={styles.sampleGroup}>
          <div className={styles.sampleGroupHeader}>
            <span>{group.title}</span>
            <strong>{group.rows.length}</strong>
            {group.mode !== "single" ? (
              <button className={styles.sampleRemoveVariable} type="button" onClick={() => removeVariable(group.mode as SampleGrouping)}>
                Remove variable
              </button>
            ) : null}
          </div>
      ...
      <ActionFooter className={styles.sampleActions}>
        <Button className={`${styles.sampleActionButton} ${styles.sampleSecondaryAction}`} disabled={!dirty} onClick={revertDraft}>Revert</Button>
        <Button className={`${styles.sampleActionButton} ${styles.samplePrimaryAction}`} variant="primary" disabled={!dirty} onClick={saveDraft}>Save</Button>
      </ActionFooter>
    </section>
  );
}
...
function synthFallbackWaveform(ref: MutableRefObject<AudioContext | null>, instrument: Instrument): AudioWaveformSummary | null {
...
function getPreviewContext(ref: MutableRefObject<AudioContext | null>): AudioContext {
