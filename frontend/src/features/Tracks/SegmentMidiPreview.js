 ...
 */
export function SegmentMidiPreview({ segment }) {
    ...
    return (_jsx("svg", { className: styles.svg, viewBox: "0 0 100 100", preserveAspectRatio: "none", "aria-hidden": true, children: notes.map((n, i) => {
            const x = (n.startBeat / segLen) * 100;
            const w = Math.max(0.5, (n.lengthBeats / segLen) * 100);
            const y = (1 - (n.pitch - minP) / pitchRange) * 100;
            return (_jsx("rect", { x: x, y: Math.max(0, Math.min(98, y - 2)), width: Math.min(100 - x, w), height: 3, className: styles.note }, i));
        }) }));
}
