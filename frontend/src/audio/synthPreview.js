...
export function createSynthRenderState() {
    return { phase: 0, index: 0, low: 0, band: 0 };
}
export function noteFrequency(midiPitch, instrument) {
...
export function previewFrequency(instrument) {
    const octave = instrument.octave ?? 0;
    const detune = instrument.detuneCents ?? 0;
    return SYNTH_PREVIEW_BASE_HZ * Math.pow(2, octave + detune / 1200);
}
export function renderInstrumentSamples(instrument, out, sampleRate, frequency, mode, fade = false) {
...
export function createInstrumentBufferSource(ctx, instrument, durationS, frequency) {
...
export async function preloadInstrumentSample(ctx, instrument) {
    if (!instrument.sampleUrl || sampleBufferCache.has(instrument.sampleUrl))
        return;
...
export function renderInstrumentSample(instrument, state, sampleRate, frequency, mode) {
...
function resonantLowpass(input, state, sampleRate, cutoff, resonance) {
    const minHz = 50;
...
function oscillatorSample(waveform, phase, color) {
    ...
    switch (waveform) {
        ...
        case "sample":
            return (Math.sin(p * Math.PI * 2) * 0.55 +
...
function whiteNoiseSample(index) {
