                    ...
                    if (seg.payload.kind === "drum") {
                        ...
                        for (const row of seg.payload.rows) {
                            ...
                            for (let step = 0; step < seg.payload.stepCount; step++) {
                                ...
                                if (noteStart < currentBeat - 0.05 || noteStart > currentBeat + lookaheadBeats)
                                    continue;
                                ...
                                }, instrument, audio.currentTime + delayS, Math.max(0.05, Math.min(0.18, stepLengthBeats / beatsPerSecond)));
                                scheduledRef.current.add(key);
                            }
                        }
                        continue;
                    }
                    if (seg.payload.kind !== "midi" && seg.payload.kind !== "mixed")
                        continue;
                    const instrument = instruments.find((i) => i.id === seg.instrumentId) ??
